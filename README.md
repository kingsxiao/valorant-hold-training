...
- 准星随移动/开火实时扩张（动态误差可视化，对应游戏内"开火误差"选项，菜单可关）——扩张瞬时可见、收束缓动归位，收束时才是出手时机
...
- 后坐力：程序化生成的弹道表（前 9 发垂直爬升 → 高位平台 → 水平摆动；垂直分量全程不下坠——压枪量到高点封顶，只左右修），水平段结构对齐公开补丁机制：前 6 发（Vandal）/ 8 发（Phantom）水平保护无横偏，其后按 0.6s 换向节拍左右摆动渐强；跑动时垂直后坐 ×1.8（v6.11 公开改动，按移速比例介入）；停火 0.4s 立即重置（鼓励点射/急停）
...
- 后坐力弹道的爬升幅度为调校近似（官方无角度数据），结构与换向节拍对齐公开补丁说明
- 只支持平地（无楼梯/台阶）
