...
export class WeaponSystem {
  ...
  _animateHands(dt) {
    ...
    // 手部滞后回弹：手套+袖臂整组轻微后移/下旋（幅度远小于枪身后坐 → 读作握持
    // 压缩）。旋转取负号（2026-09-07）：手与枪同挂 vmHolder、后坐本身无相对运动，
    // 唯一相对位移来自这里——正值上旋会把手扫进护木/握把（连发峰值 flinch 0.09
    // 时 11-13 采样顶点穿插，左手腕口为主）；负号=手相对上抬的枪下垂让位，
    // 物理上也更正确（滞后方向），连发全程逐帧采样 0 穿插
    if (ha.group) {
