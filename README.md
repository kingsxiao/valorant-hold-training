...
- 敌方枪声为 HRTF 空间音频（听者方位换算与相机矩阵严格一致，可听声辨位）；Bot 不发出脚步声，脚步音仅为玩家自己的跑动反馈
- WebGL 上下文丢失自动暂停并提示，恢复后重建材质继续

## 训练模式

只有一种纯粹的架枪对枪训练：开局 3 秒倒计时（GO 后才开始计时），随机延迟后 Bot 以全速从巷道缺口（A/B）拉出——带概率急停与"露头即缩"的 jiggle-peek（拉到中段折返缩回，逼你守住准星等第二拉）；在"击杀时限"内没打中，Bot 就缩回墙后、记一次对枪败，你继续架下一波——**没有死亡/掉血设定**，训练节奏永不中断，练先开枪能力。中场木箱可换点位架枪，地面距离标线帮助测距。

**训练开关**（菜单"参数"区）：
- 渐进难度：每次击杀后 Bot 出现延迟 ×0.93、横移速度 ×1.02（有保底/封顶），一局之内越打越快
- 双缺口压力：A/B 缺口各一个独立槽位出人（B 错开 1.5s），练交叉火力下的目标选择与转火
...
- 对枪失败：Bot 原地开火还击（枪口焰/曳光 + 空间枪声 + 受击闷响）后缩回，屏幕边缘红弧指向来源方位——只是失败提示，不掉血
...
质量保障：
- `npm run verify` = ESLint + Vitest（64 用例）+ 生产构建；GitHub Actions CI 对每次 push/PR 运行同款门禁
