...
export const GLOVE_POSES = {
  ...
  vandal: {
    handR: {
      mirror: true,
      // 腕锚 2026-09-11 重校：GLB 原始单位换版后（旧表按旧单位标定：护木在
      // x -2.4..-1.2、全长约 3.5u；现全长 1.285u，×~0.37），旧锚 [1.7,0.1,-0.317]
      // 投到枪外悬空 —— 腰射灰手套浮在屏中、开镜手悬屏心即此。
      // 现按当前几何切片重推：握把凸起 x 0.22..0.30、顶口 y≈0.00（x≈0.21）、
      // 右侧面 z≈-0.038；腕置握把顶右后（顶口后 0.03、右让 0.017 出右面）
      wrist: [0.24, 0.005, -0.055],
      fDes: [-0.3, -0.88, -0.36],
      sDes: [0.2, 0.8, -0.55],
    ...
    handL: {
      // 腕锚 2026-09-11 重校（同 handR 单位换版）：护木 x -0.38..-0.18、底面
      // y≈0.08、管轴 y=0.108；腕置护木底下中段（x 中点、贴底面微下、z 居中）
      wrist: [-0.27, 0.065, 0],
      fDes: [-0.2, -0.1, -0.97],
      sDes: [0.95, 0.25, -0.2],
  ...
  phantom: {
    handR: {
      mirror: true,
      // 腕锚 2026-09-11 重校（GLB 单位换版，同 vandal 条目说明）：握把凸起
      // x 0.237..0.31、顶口 (x≈0.23, y≈-0.03)、右侧面 z≈-0.021；腕置握把顶
      // 右后，右让出右面。旧值 [0.42,0.1,-0.09] 为旧单位标定，已失效
      wrist: [0.25, -0.02, -0.05],
      fDes: [-0.3, -0.88, -0.36],
      sDes: [0.2, 0.8, -0.55],
    ...
    handL: {
      // 腕锚 2026-09-11 重校（单位换版）：护木 x -0.31..-0.16、底面 y≈0.025、
      // 管轴 y=0.079；腕置护木底下中段，贴底面微下、z 居中
      wrist: [-0.22, 0.02, 0],
      fDes: [-0.2, -0.1, -0.97],
      sDes: [0.95, 0.25, -0.2],
